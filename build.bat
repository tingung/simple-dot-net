...
"%SOURCES_DIR%\Nuget\Nuget.exe" restore "%SOURCES_DIR%\simple-dot-net.sln"
if %ERRORLEVEL% neq 0 (
	echo Warning: Nuget restore operation finished with errors, exit code: %ERRORLEVEL%>&2
)

%MSBUILD_COMMAND% "%SOURCES_DIR%\simple-dot-net.sln" "/p:OutputPath=%OUTPUT_DIR%" /t:Build /p:Configuration=%BUILDTARGET%
if %ERRORLEVEL% neq 0 (
